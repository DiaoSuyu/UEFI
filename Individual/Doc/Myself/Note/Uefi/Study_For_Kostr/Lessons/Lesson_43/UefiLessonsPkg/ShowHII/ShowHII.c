...
VOID
ParseHiiPackageLists (
  IN EFI_HII_PACKAGE_LIST_HEADER  *HiiDatabase,
  IN UINTN                        HiiDatabaseSize
  )
{
    UINTN Index = 0;
    EFI_HII_PACKAGE_LIST_HEADER *HiiPackageListHeader = HiiDatabase;
    while ((UINTN)HiiPackageListHeader < ((UINTN)HiiDatabase + HiiDatabaseSize)) {
        ...
        Print(L"PackageList[%04d]: GUID = %g; size = 0x%X\n", Index++, HiiPackageListHeader->PackageListGuid, HiiPackageListHeader->PackageLength);
        ...
        while ((UINTN)HiiPackageHeader < ((UINTN)HiiPackageListHeader + HiiPackageListSize)) {
            Print(L"    Package[%04d]: type = %s; size = 0x%X\n", Number++, PackageType(HiiPackageHeader->Type), HiiPackageHeader->Length);
            // Go to next Package
  ...
  )
{
    ...
    Status = HiiDbProtocol->ExportPackageLists (
        HiiDbProtocol,
