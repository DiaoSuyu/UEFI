  ...
  ) 
{
    ...
    EFI_STATUS Status = ShellProtocol->OpenFileByName (
        FileName,
        &FileHandle,
...
EFI_STATUS
CheckSubtables (
  IN EFI_ACPI_6_5_COMMON_HEADER  *table
  )
{
    EFI_STATUS Status;
    if (
        ((CHAR8)((table->Signature >>  0) & 0xFF) == 'F') &&
        ...
        if (
            ...
            ((CHAR8)((DSDT->Signature >> 24) & 0xFF) == 'T')
                  ...
                  Status = SaveACPITable(DSDT->Signature, DSDT, DSDT->Length);
                  if (EFI_ERROR(Status)) {
                      Print(L"Error to SaveACPITable: %r\n", Status);
                      return Status;
                  }
        } else {
        ...
        if (
            ...
            ((CHAR8)((FACS->Signature >> 24) & 0xFF) == 'S')
                  ...
                  Status = SaveACPITable(FACS->Signature, FACS, FACS->Length);
                  if (EFI_ERROR(Status)) {
                      Print(L"Error to SaveACPITable: %r\n", Status);
                      return Status;
                  }
  ...
  )
{
    ...
    while (offset < XSDT->Length) {
        ...
        EFI_ACPI_6_5_COMMON_HEADER *table = (EFI_ACPI_6_5_COMMON_HEADER*)(*table_address);
        Print( \
            L"\t%c%c%c%c table is placed at address %p with length 0x%x\n",  \
        ...
        Status = CheckSubtables(table);
        if (EFI_ERROR(Status)) {
            Print(L"Error to CheckSubtables: %r\n", Status);
            return Status;
        }

        Status = SaveACPITable(table->Signature, table, table->Length);
        if (EFI_ERROR(Status)) {
            Print(L"Error to SaveACPITable: %r\n", Status);
            return Status;
        }
